...
| `Can't Delete Screenshot After Submit` | Screenshots are locked once submitted for review | `asc cancel-review` → fix → resubmit (minutes with the same build) |
| deliver double-uploads screenshots (same filename twice in one set) | deliver upload race — suspect it when 'Successfully uploaded all screenshots' logs twice | **Always verify per-locale counts via the API before submitting.** Remove dupes by filename with `DELETE /v1/appScreenshots/{id}` |
| `What's New ... can't contain the following character(s)` | ASC rejects certain emoji in the whatsNew field (measured with 🍜) | Strip the emoji from release_notes.txt — if you must use emoji, stick to common ones (⚡️) and be ready to remove them |
